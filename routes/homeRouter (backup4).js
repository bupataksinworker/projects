...
const moment = require('moment'); // นำเข้า moment
...
const fetchSales = async (dateStart, dateEnd) => {
    ...
    try {
        ...
        const salesWithTotalAmounts = await Promise.all(sales.map(async sale => {
            ...
            return { ...sale.toObject(), totalValues };
        }));
...
router.get('/home', async (req, res) => {
    try {
        ...
        const salesData = await fetchSales(1);
        const saleEntriesData = await fetchSaleEntries(dateStart, dateEnd);

        res.render('home', {
            salesData: JSON.stringify(salesData.sales),
            saleEntriesData: JSON.stringify(saleEntriesData),
        });
    } catch (error) {
