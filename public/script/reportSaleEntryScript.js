...
function updateTable(saleEntries, page = 1) {
    ...
    paginatedEntries.forEach((entry, index) => {
        ...
        // กำหนดค่า weight, price, และ sale ตาม entryStatus
        const weight = (entry.entryStatus === 'N' || entry.entryStatus === 'C') ? entry.openWeight : entry.closeWeight;
        const price = (entry.entryStatus === 'N' || entry.entryStatus === 'C') ? entry.openPrice : entry.closePrice;
        const sale = (entry.entryStatus === 'N' || entry.entryStatus === 'C') ? entry.totalPreSale : entry.totalSale;
        ...
        row.innerHTML = `
            ...
            <td class="setNumber">${addCommas(weight)}</td>
            <td class="setNumber">${addCommas(price)}</td>
            <td class="setNumber">${addCommas(sale)}</td>
